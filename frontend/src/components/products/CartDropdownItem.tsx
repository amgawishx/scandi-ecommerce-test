...
export const CartDropdownItem: React.FC<CartDropdownItemProps> = ({
  ...
  const groupedAttributes = (data?.product?.attributes || []).reduce(
    (acc: any, attr: any) => {
  ...
  return (
    <div
      className="cart-item"
      data-testid={`product-${item.name.replaceAll(" ", "-").toLowerCase()}`}
    >
      ...
      <div className="cart-item-details">
        ...
        {Object.entries(groupedAttributes).map(
          ([name, { type, values }]: any) => (
            <div
              key={name}
              className="cart-attribute-section"
              data-testid={`cart-item-attribute-${name.replaceAll(" ", "-").toLowerCase()}`}
            >
              <h4 className="cart-attribute-name">{name}:</h4>
              <div className="cart-attribute-values">
                {values.map(({ id, value, displayValue }: any) => {
                  const isSelected =
                    item.selectedAttributes[name]?.value === value;

                  return (
                    <div
                      key={id}
                      className={`cart-attribute-button ${type === "swatch" ? "swatch" : ""} ${isSelected ? "selected" : ""}`}
                      data-testid={`product-item-attribute-${name.replaceAll(" ", "-").toLowerCase()}-${displayValue.replaceAll(" ", "-")}${isSelected ? "-selected" : ""}`}
                      style={
                        type === "swatch"
